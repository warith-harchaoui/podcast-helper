...
- **MCP surface** (`podcast_helper.mcp`, `[mcp]` extra, entry point
  `podcast-helper-mcp`): exposes the existing FastAPI app (`feed`, `latest`,
  `probe`, `stream`, `record`) as MCP tools via `fastapi-mcp`, so any
  MCP-aware agent host can call them directly. Mirrors the pattern already
  shipped in `standpoint` / `vocal-helper` / `md2star` / `os-helper`. Closes
  the CLI/API/MCP surface gap flagged in `ai-helpers/.private/do.md` §7.
...
- **MCP surface dropped.** `fastapi-mcp`'s latest release (0.4.0) is
  ...
  Removed `podcast_helper/mcp.py`, the `podcast-helper-mcp` entry point, the
  `mcp` extra, and every doc/skill mention. The library, both CLIs, the
  FastAPI HTTP surface, and the browser GUI are unaffected — podcast-helper
...
- Complete Numpy-style docstrings across the package per CODING.md (CLI
  handlers, feed/streaming/api private helpers).
...
- Harmonize README/LISEZMOI to the AI Helpers common structure (single H1,
  source install pinned to v0.3.5, PyPI-coming-soon note); no code changes.
...
- Bump youtube-helper pin to v1.3.2 (audio-helper v1.5.4: any-format decode).
