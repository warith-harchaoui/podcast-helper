"""
Smoke test for the MCP surface (``podcast_helper.mcp``).
...
def test_mcp_module_imports_and_exposes_app():
    ...
    from podcast_helper import mcp as mcp_module

    assert hasattr(mcp_module, "app"), "podcast_helper.mcp must re-expose `app`."
    assert hasattr(mcp_module, "mcp"), "podcast_helper.mcp must expose the `mcp` handler."


def test_main_entrypoint_is_callable():
    """The ``podcast-helper-mcp`` console entry point should be a callable."""
    from podcast_helper.mcp import main
